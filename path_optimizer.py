...
class PathOptimizer:
    ...
    def optimize_spiral(self, xf, yf, tf):
        ...
        res = scipy.optimize.minimize(
            fun=self.objective, x0=p0, method="L-BFGS-B", jac=self.objective_grad, bounds=bounds
        )
        # ------------------------------------------------------------------
    ...
    def thetaf(self, a, b, c, d, s):
        ...
        A = a * s
        B = b * 0.5 * s_square
        C = c / 3 * np.multiply(s_square, s)
        D = d * 0.25 * np.multiply(s_square, s_square)
        thetas = A + B + C + D
        return thetas
        # ------------------------------------------------------------------
    ...
    def objective_grad(self, p):
        ...
        p = [0.0, p[0], p[1], 0.0, p[2]]
        return np.add(
            np.add(np.add(self.fbe_grad(p), np.multiply(25, self.fxf_grad(p))), np.multiply(25, self.fyf_grad(p))),
